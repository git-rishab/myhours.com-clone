...
document.getElementById("add").addEventListener("click",()=>{
    ...
    tr.innerHTML = `<td><input type="text" placeholder="Project Name" class="first-col"></td>
    <td><input type="text" placeholder="Task" class="second-col"></td>
    <td><input type="text" placeholder="hh:mm" class="day-time first" ></td>
    <td><input type="text" placeholder="hh:mm" class="day-time second"></td>
...
function total() {
    let project = document.getElementsByClassName("first-col");
    let total = document.getElementsByClassName("add-time-total");

    let timeStamps = {};

    for(let i = 0; i < project.length; i++){
        if(project[i].value){
            if(!timeStamps[project[i].value.split(" ").join("")]){
                timeStamps[project[i].value.split(" ").join("")] = {};
                timeStamps[project[i].value.split(" ").join("")].total = +total[i].innerText.split(":").join("");
            } else {
                timeStamps[project[i].value.split(" ").join("")].total += +total[i].innerText.split(":").join("");
            }
        }
